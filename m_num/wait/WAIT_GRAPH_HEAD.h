...
double Target_Moving_Dist_Threshold = 20000; //Distance of target moving
...
double Total_Distance_Saved = 0; // Total saved moveing distance of nodes
char Result_File[256]; //File address for saving resuilts
...
/* Schedule the mobile nodes according to expectations */
{
    ...
    //    return;
    //}
    /* If no move or move to other vertex */
    for (int i = 0; i < Mobile_Node_Num; ++i) {
        if (i == tracking_index) {
            continue;
        }
